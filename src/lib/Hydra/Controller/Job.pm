...
sub shield : Chained('job') PathPart('shield') Args(0) {
    ...
    my $lastBuild = $c->stash->{jobset}->builds->find({ job => $job, finished => 1 },
...
sub prometheus : Chained('job') PathPart('prometheus') Args(0) {
    ...
    my $lastBuild = $c->stash->{jobset}->builds->find({ job => $c->stash->{job}, finished => 1 },
        { order_by => 'id DESC', rows => 1, columns => [@buildListColumns] });
    ...
    )->labels($c->stash->{project}->name, $c->stash->{jobset}->name, $c->stash->{job},)->inc($lastBuild->stoptime);

    $prometheus->new_gauge(
        name   => "hydra_job_failed",
        help   => "Record if the most recent version of this job failed (1 means failed)",
        labels => [ "project", "jobset", "job" ]
    )->labels($c->stash->{project}->name, $c->stash->{jobset}->name, $c->stash->{job},)
...
sub overview : Chained('job') PathPart('') Args(0) {
    ...
    $c->stash->{queuedBuilds} = [ $c->stash->{jobset}
          ->builds->search({ job => $c->stash->{job}, finished => 0 }, { order_by => [ "priority DESC", "id" ] }) ];

    # If this is an aggregate job, then get its constituents.
    my @constituents = $c->model('DB::Builds')->search(
        {
            aggregate => {
                -in => $c->stash->{jobset}->builds->search({ job => $c->stash->{job} },
                    { columns => ["id"], order_by => "id desc", rows => 15 })->as_query
            }
        },
        {
    ...
    foreach my $agg (keys %$aggregates) {
        ...
        $aggregates->{$agg}->{build} = $c->model('DB::Builds')->find({ id => $agg }, { columns => [@buildListColumns] })
          or die;
    }
...
sub metrics_tab : Chained('job') PathPart('metric-tab') Args(0) {
    ...
    $c->stash->{metrics}  = [
        $c->stash->{jobset}->buildmetrics->search(
            { job    => $c->stash->{job} },
...
sub build_times : Chained('job') PathPart('build-times') Args(0) {
    ...
    $self->status_ok($c,
        entity =>
          [ map { { id => $_->id, timestamp => $_->timestamp, value => $_->get_column('actualBuildTime') } } @res ]);
}
...
sub closure_sizes : Chained('job') PathPart('closure-sizes') Args(0) {
    ...
    my @res =
      $c->stash->{jobset}
      ->builds->search({ job => $c->stash->{job}, finished => 1, buildstatus => 0, closuresize => { '!=', 0 } },
        { order_by => "id", columns => [ "id", "timestamp", "closuresize" ] });
    $self->status_ok($c,
        entity => [ map { { id => $_->id, timestamp => $_->timestamp, value => $_->closuresize } } @res ]);
}
...
sub output_sizes : Chained('job') PathPart('output-sizes') Args(0) {
    ...
    my @res =
      $c->stash->{jobset}
      ->builds->search({ job => $c->stash->{job}, finished => 1, buildstatus => 0, size => { '!=', 0 } },
...
sub metric : Chained('job') PathPart('metric') Args(1) {
    ...
    my @res = $c->stash->{jobset}->buildmetrics->search({ job => $c->stash->{job}, name => $metricName },
        { order_by => "timestamp", columns => [ "build", "name", "timestamp", "value", "unit" ] });

    $self->status_ok(
        $c,
        entity => [
            map { { id => $_->get_column("build"), timestamp => $_->timestamp, value => $_->value, unit => $_->unit } }
              @res
        ]
    );
}
