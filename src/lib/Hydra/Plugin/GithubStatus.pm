...
sub extractGithubArgsFromFlake {
    my ($flakeref) = @_;
    if (   $flakeref =~ m!github:([^/]+)/([^/]+)/([[:xdigit:]]{40})$!
        or $flakeref =~ m!git\+ssh://git\@github.com/([^/]+)/([^/]+)\?.*rev=([[:xdigit:]]{40})$!)
    {
        return {
            owner => $1,
            repo  => $2,
    ...
    foreach my $build ($topbuild, @{$dependents}) {
        ...
        foreach my $conf (@config) {
            ...
            my $cachingSendStatus = sub {
                ...
                sendStatus($owner, $repo, $rev, $ua, $body,
                    ($self->{config}->{github_authorization}->{$owner} // $conf->{authorization}));
            };
            ...
            while (my $eval = $evals->next) {
                ...
                if (defined $eval->flake) {
                    ...
                    if (defined($githubArgs)) {
                        $cachingSendStatus->("src", $githubArgs->{"owner"}, $githubArgs->{"repo"},
                            $githubArgs->{"rev"});
                    }
                    else {
                    ...
                    foreach my $input (@inputs) {
                        ...
                        if (defined($githubArgs)) {
                            $cachingSendStatus->(
                                $input,                $githubArgs->{"owner"},
                                $githubArgs->{"repo"}, $githubArgs->{"rev"}
                            );
                        }
                        else {
                            print STDERR
                              "Evaluation $eval: Can't parse input $input\'s URI, skipping GitHub status update\n";
                        }
                    }
                }
            }
