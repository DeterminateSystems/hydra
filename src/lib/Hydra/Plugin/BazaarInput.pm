...
sub fetchInput {
    ...
    (my $cachedInput) = $self->{db}->resultset('CachedBazaarInputs')->search({ uri => $uri, revision => $revision });
    ...
    if (defined $cachedInput && isValidPath($cachedInput->storepath)) {
        ...
        (my $res, $stdout, $stderr) = captureStdoutStderr(1200, "nix-prefetch-bzr", $uri, $revision);
