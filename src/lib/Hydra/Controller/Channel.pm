...
sub channel : Chained('/') PathPart('channel/custom') CaptureArgs(3) {
    ...
    notFound($c, "Channel $channelName either doesn't exist " . "or was never built successfully.")
      if !$lastSuccessful;
...
sub overview : Chained('channel') PathPart('') Args(0) {
    ...
    $c->stash->{constituents} = [ $c->stash->{lastSuccessful}->constituents_->search({}, { order_by => ["job"] }) ];
...
sub nixexprs : Chained('channel') PathPart('') Args(1) {
    ...
    my $product = $c->stash->{lastSuccessful}->buildproducts->find({ type => "channel", name => $productName });

    my $url = $c->uri_for(
        $c->controller("Build")->action_for("download"),
        [ $c->stash->{lastSuccessful}->id ],
        $product->productnr, $productName
    );
