...
sub fetchInput {
    ...
    ($res, $stdout, $stderr) = captureStdoutStderr(600, ("darcs", "pull", "-a", "--repodir", $clonePath, "$uri"));
    die "Error fetching latest change from darcs repo at `$uri':\n$stderr" if $res;

    ($res, $stdout, $stderr) =
      captureStdoutStderr(600, ("darcs", "changes", "--last", "1", "--xml", "--repodir", $clonePath));
    ...
    my $cachedInput;
    ($cachedInput) =
      $self->{db}->resultset('CachedDarcsInputs')->search({ uri => $uri, revision => $revision }, { rows => 1 });
    ...
    if (defined $cachedInput && isValidPath($cachedInput->storepath)) {
        ...
        (system "darcs", "get", "--lazy", $clonePath, "$tmpDir/export", "--quiet", "--to-match", "hash $revision") == 0
          or die "darcs export failed";
        $revCount = `darcs changes --count --repodir $tmpDir/export`;
        chomp $revCount;
